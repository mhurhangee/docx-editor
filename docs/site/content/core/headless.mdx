...
| `createContentControl(doc, target, props?, options?)` | Wrap a text span (inline) or block range in a new control; returns `{ doc, control }` |
...
`createContentControl` and the `setContent*` mutators reach **inline** controls (including inside table cells, and headers/footers via `{ scope: 'all' }`), not just block-level controls.

Errors are typed: `ContentControlNotFoundError`, `ContentControlLockedError`, `ContentControlTypeError`, `ContentControlBoundError`, `ContentControlValueError`, `ContentControlCreateError`, `RepeatingSectionError`.
